...
export function buildTranslatePrompt(text: string, language: string): PromptMessages {
  const system = [
    ...
    "It may contain Dict/Dict+, dynamic inline variables using <term>=#<letter><digit>, and readable labels such as Status:, Context:, Action:, Risk:, Outcome:, Constraint:, and Proof:.",
    "Older /distill output may also contain one-letter prefixes: S=state/status, C=cause/context, D=action/decision, R=risk/blocker, O=outcome/output, N=constraint/no-go, P=pass criteria/proof.",
